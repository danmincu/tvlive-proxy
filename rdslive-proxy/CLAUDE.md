...
**Browser playback:** raw HLS doesn't play natively in Chrome/Firefox, so `IndexPage` loads **hls.js** from a CDN and falls back to native HLS on Safari. The player tracks `currentPath` (`/stream.m3u8` or `/dvr.m3u8?vod=1`) so the cast button casts whatever is playing. The **DVR button loads `/dvr.m3u8?vod=1`** (ENDLIST → VOD) so hls.js treats it as a fixed, fully-seekable recording and never reloads/snaps to the live edge — a plain live playlist (no ENDLIST) reloads every target-duration and re-anchors to live, which makes scrubbing back impossible. The VOD is a snapshot up to the click moment; re-click DVR for newer recordings. The stall-watchdog and health auto-resume are **live-mode only** so they never yank the user out of a DVR recording.
