...
- **`GET /health`** — upstream health JSON (`ok`, `error`, `failures`, `lastOkUtc`, `lastSegmentUtc`) from `StreamHealth`, which both the recorder and `/stream.m3u8` feed. The player polls it and shows a red banner when the source is down/changed.
